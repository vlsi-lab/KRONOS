...
mcu-gen:
	$(MAKE) -f $(XHEEP_MAKE) $(MAKECMDGOALS) CPU=cv32e40p BUS=NtoM MEMORY_BANKS=16 EXTERNAL_DOMAINS=$(EXTERNAL_DOMAINS)
...
run-$(TESTS):
	cd ./build/vlsi_polito_mcu_kronos_0/sim-modelsim; \
	make run-gui PLUSARGS="c firmware=../../../sw/applications/$(TESTS)/main.hex"; \
	cat uart0.log; \
	cd ../../..;
