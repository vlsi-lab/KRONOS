...
module keccak_reg_top #(
  ...
  assign data_0_we = addr_hit[0] & reg_we & ~wr_err;
  assign data_0_wd = reg_wdata[31:0];

  assign data_1_we = addr_hit[1] & reg_we & ~wr_err;
  assign data_1_wd = reg_wdata[31:0];

  assign data_2_we = addr_hit[2] & reg_we & ~wr_err;
  assign data_2_wd = reg_wdata[31:0];

  assign data_3_we = addr_hit[3] & reg_we & ~wr_err;
  assign data_3_wd = reg_wdata[31:0];

  assign data_4_we = addr_hit[4] & reg_we & ~wr_err;
  assign data_4_wd = reg_wdata[31:0];

  assign data_5_we = addr_hit[5] & reg_we & ~wr_err;
  assign data_5_wd = reg_wdata[31:0];

  assign data_6_we = addr_hit[6] & reg_we & ~wr_err;
  assign data_6_wd = reg_wdata[31:0];

  assign data_7_we = addr_hit[7] & reg_we & ~wr_err;
  assign data_7_wd = reg_wdata[31:0];

  assign data_8_we = addr_hit[8] & reg_we & ~wr_err;
  assign data_8_wd = reg_wdata[31:0];

  assign data_9_we = addr_hit[9] & reg_we & ~wr_err;
  assign data_9_wd = reg_wdata[31:0];

  assign data_10_we = addr_hit[10] & reg_we & ~wr_err;
  assign data_10_wd = reg_wdata[31:0];

  assign data_11_we = addr_hit[11] & reg_we & ~wr_err;
  assign data_11_wd = reg_wdata[31:0];

  assign data_12_we = addr_hit[12] & reg_we & ~wr_err;
  assign data_12_wd = reg_wdata[31:0];

  assign data_13_we = addr_hit[13] & reg_we & ~wr_err;
  assign data_13_wd = reg_wdata[31:0];

  assign data_14_we = addr_hit[14] & reg_we & ~wr_err;
  assign data_14_wd = reg_wdata[31:0];

  assign data_15_we = addr_hit[15] & reg_we & ~wr_err;
  assign data_15_wd = reg_wdata[31:0];

  assign data_16_we = addr_hit[16] & reg_we & ~wr_err;
  assign data_16_wd = reg_wdata[31:0];

  assign data_17_we = addr_hit[17] & reg_we & ~wr_err;
  assign data_17_wd = reg_wdata[31:0];

  assign data_18_we = addr_hit[18] & reg_we & ~wr_err;
  assign data_18_wd = reg_wdata[31:0];

  assign data_19_we = addr_hit[19] & reg_we & ~wr_err;
  assign data_19_wd = reg_wdata[31:0];

  assign data_20_we = addr_hit[20] & reg_we & ~wr_err;
  assign data_20_wd = reg_wdata[31:0];

  assign data_21_we = addr_hit[21] & reg_we & ~wr_err;
  assign data_21_wd = reg_wdata[31:0];

  assign data_22_we = addr_hit[22] & reg_we & ~wr_err;
  assign data_22_wd = reg_wdata[31:0];

  assign data_23_we = addr_hit[23] & reg_we & ~wr_err;
  assign data_23_wd = reg_wdata[31:0];

  assign data_24_we = addr_hit[24] & reg_we & ~wr_err;
  assign data_24_wd = reg_wdata[31:0];

  assign data_25_we = addr_hit[25] & reg_we & ~wr_err;
  assign data_25_wd = reg_wdata[31:0];

  assign data_26_we = addr_hit[26] & reg_we & ~wr_err;
  assign data_26_wd = reg_wdata[31:0];

  assign data_27_we = addr_hit[27] & reg_we & ~wr_err;
  assign data_27_wd = reg_wdata[31:0];

  assign data_28_we = addr_hit[28] & reg_we & ~wr_err;
  assign data_28_wd = reg_wdata[31:0];

  assign data_29_we = addr_hit[29] & reg_we & ~wr_err;
  assign data_29_wd = reg_wdata[31:0];

  assign data_30_we = addr_hit[30] & reg_we & ~wr_err;
  assign data_30_wd = reg_wdata[31:0];

  assign data_31_we = addr_hit[31] & reg_we & ~wr_err;
  assign data_31_wd = reg_wdata[31:0];

  assign data_32_we = addr_hit[32] & reg_we & ~wr_err;
  assign data_32_wd = reg_wdata[31:0];

  assign data_33_we = addr_hit[33] & reg_we & ~wr_err;
  assign data_33_wd = reg_wdata[31:0];

  assign data_34_we = addr_hit[34] & reg_we & ~wr_err;
  assign data_34_wd = reg_wdata[31:0];

  assign data_35_we = addr_hit[35] & reg_we & ~wr_err;
  assign data_35_wd = reg_wdata[31:0];

  assign data_36_we = addr_hit[36] & reg_we & ~wr_err;
  assign data_36_wd = reg_wdata[31:0];

  assign data_37_we = addr_hit[37] & reg_we & ~wr_err;
  assign data_37_wd = reg_wdata[31:0];

  assign data_38_we = addr_hit[38] & reg_we & ~wr_err;
  assign data_38_wd = reg_wdata[31:0];

  assign data_39_we = addr_hit[39] & reg_we & ~wr_err;
  assign data_39_wd = reg_wdata[31:0];

  assign data_40_we = addr_hit[40] & reg_we & ~wr_err;
  assign data_40_wd = reg_wdata[31:0];

  assign data_41_we = addr_hit[41] & reg_we & ~wr_err;
  assign data_41_wd = reg_wdata[31:0];

  assign data_42_we = addr_hit[42] & reg_we & ~wr_err;
  assign data_42_wd = reg_wdata[31:0];

  assign data_43_we = addr_hit[43] & reg_we & ~wr_err;
  assign data_43_wd = reg_wdata[31:0];

  assign data_44_we = addr_hit[44] & reg_we & ~wr_err;
  assign data_44_wd = reg_wdata[31:0];

  assign data_45_we = addr_hit[45] & reg_we & ~wr_err;
  assign data_45_wd = reg_wdata[31:0];

  assign data_46_we = addr_hit[46] & reg_we & ~wr_err;
  assign data_46_wd = reg_wdata[31:0];

  assign data_47_we = addr_hit[47] & reg_we & ~wr_err;
  assign data_47_wd = reg_wdata[31:0];

  assign data_48_we = addr_hit[48] & reg_we & ~wr_err;
  assign data_48_wd = reg_wdata[31:0];

  assign data_49_we = addr_hit[49] & reg_we & ~wr_err;
  assign data_49_wd = reg_wdata[31:0];

  assign ctrl_we = addr_hit[50] & reg_we & ~wr_err;
